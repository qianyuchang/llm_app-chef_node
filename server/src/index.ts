...
app.post('/api/ai/optimize-image', async (req, res) => {
  ...
  try {
      ...
      // 2. Strip prefix for API
      const base64Data = image.replace(/^data:image\/\w+;base64,/, "");

      console.log(`Optimizing image type: ${mimeType}, length: ${base64Data.length}`);

      const prompt = "Enhance this food photo. Make it look like professional high-end food photography with warm lighting, appetizing glossy texture, and studio quality. Keep the food items and composition exactly the same, but significantly improve the color grading, contrast and sharpness to make it mouth-watering.";
      ...
      if (newImageBase64) {
          // Re-attach prefix for frontend display (Gemini usually returns PNG/JPEG raw bytes)
          // We'll assume JPEG for the output unless we check the response headers/metadata, 
          // but usually keeping it generic or just data:image/png is fine if the bytes are valid.
          // Let's assume JPEG/PNG compatible prefix.
